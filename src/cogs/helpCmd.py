...
class EmbedHelp(dpy_cmds.DefaultHelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        if not permissions.embed_links:
            await dest.send(f"Error! Stalk Market Bot must have the `Embed Links` permission in this channel to continue. Please give Stalk Market Bot the `Embed Links` permission and try again."
                            f"\nIf you need assistance, please talk to Amadea System")
            return

        embed = discord.Embed(title='Stalk Market Bot Help', color=help_embed_color)

        embed.add_field(name="What is Stalk Market Bot for?",
                        value="This bot helps keeps track of your turnip prices and predicts what they will be throughout the week.")

        embed.set_footer(text="Created by Luna, Hibiki, and Fluttershy aka Amadea System (Hibiki#8792) "
                              "| Github: https://github.com/amadea-system/StalkMarketBot/")
    ...
    async def send_command_help(self, command: dpy_cmds.Command):
        ...
        embed = discord.Embed(title=f'{command_name}  -  Stalk Market Help', color=help_embed_color)
        embed.description = "\n".join(embed_descrip)
